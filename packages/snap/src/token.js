...
const { ERC721, ERC1155, ERC20 } = _interfaces(provider);
...
function computeInterfaceId(contract, methods) {
  ...
  const unmatched = methods.filter(
    (method) => !interfaces.find((entry) => entry.name === method),
  );
  if (unmatched.length > 0) {
    throw new Error(`Missing methods: ${unmatched.join(', ')}`);
  }
...
async function isERC721(address) {
  ...
  try {
    ...
    if (EXPECTED_ERRORS.some((msg) => e.message.includes(msg))) {
      return false;
    }
    throw e;
  }
...
module.exports = {
  identify,
  is: { ERC20: isERC20, ERC721: isERC721, ERC1155: isERC1155 },
};
...
async function test() {
  ...
  async function assert(pass, fail, fn, addresses) {
    let res = 0;
    for (const [addr, result] of await Promise.all(
      addresses.map(async (addr_) => [addr_, await fn(addr_)]),
    )) {
      if (!result) {
        res += 1;
        console.error(`\x1b[31m> [X]\x1b[0m ${addr} ${fail}`);
      } else {
        console.log(`  \x1b[32m[+]\x1b[0m ${addr} ${pass}`);
      }
    }
    return res;
  }
